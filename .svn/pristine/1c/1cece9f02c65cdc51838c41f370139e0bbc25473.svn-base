...
<script>
import { getAppointList, PostappointData } from "@/api/apiz";
...
export default {
  ...
  mounted() {
    ...
    if (sessionStorage.getItem("hospitalInfo")) {
      this.formData.hospital = JSON.parse(
        sessionStorage.getItem("hospitalInfo")
      ).hospitalName;
      this.formData.uploadData.hospitalId = JSON.parse(
        sessionStorage.getItem("hospitalInfo")
      ).hospitalId;
    }
    if (sessionStorage.getItem("departmentInfo")) {
      ...
      this.formData.uploadData.departmentId=JSON.parse(
        sessionStorage.getItem("departmentInfo")
      ).id;
    }
  },
  methods: {
    ...
    selectDepartment() {
      if (this.formData.uploadData.hospitalId) {
        this.leavePageFlag = false;
        this.$nextTick(() => {
          sessionStorage.setItem("formData", JSON.stringify(this.formData));
