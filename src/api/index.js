...
export function getMedicalInfo(id) {
 return getData('/hjjk/healthTeam/info',{userId:id})
}
...
//用药动态-最近用药
export function getRecentMedical(memberId) {
  // let query={
  //   w: [{ k: "userId", v: userId, m: "eq" }],
  //   o:[{k:"crtTm",t:"desc"}],
  //   p: { n: 1, s: 10 }
  // }
  return getData('/hjjk/medRecord/recentMed',{memberId:memberId} )
}
//用药动态-获取用户用药列表
...
export function getIndicators(){
  let query = {"w":[{"k":"userId","v":"4745078308721664","m":"eq"}],"o":[{"k":"crtTm","t":"desc"}],"p":{"n":1,"s":10}}
