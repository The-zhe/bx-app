 ...
 * @Last Modified time: 2020-05-20 11:13:34
 */
  ...
  <div class="overlay" @click="overlayHide" v-show="overlayShow">
    ...
    <div class="content">
      ...
      <div class="navMenu"  :class="{'menuShow':navShow}">
        <div class="nav_item" :class="{'activeMenu':index==activeMenuIndex}" @click.stop="wxToPage(index,item)" v-for="(item,index) in wxAllmenuList">
          <div class="nav_icon">
            <img :src="item.icon"/>
          </div>
  ...
  <div v-else-if="!($route.meta.noHeader)&&(headerShow)" class="header_container" :style="{background:$route.meta.gradualHeader?'url('+gradualBackground+') no-repeat':$route.meta.headerColor?$route.meta.headerColor:'#008B84','backgroundRepeat':'no-repeat','backgroundSize':'100% 100%'}">
      <div class="content">
          ...
          <div class="navMenu" v-if="navShow">
              <div class="nav_item" @click.stop="JumpToPage(index)" v-for="(item,index) in wxAllmenuList">
                  <div class="nav_title">{{item.title}}</div>
              </div>
          </div>
      </div>
...
export default {
    ...
    data(){
        return{
          ...
          doctorMenulist:[
            ...
            {
              title:"个人中心",
              userPath:'/personalInfo',
              doctorPath:'/personalInfo',
              listIndex:7
            },
          ],
            navShow:false,
    ...
    computed:{
      wxmenuList(){
        ...
        return [menu[0],...(this.hasLogin?[]:[menu[1]]),...(this.hasLogin?[]:[menu[2]]),...(this.isDoctor?[]:[menu[3]]),...menu.slice(4,7)]
      }
    },
    methods:{
      menuHeightInit(){
      ...
      getNotReadNumberFun(){
        if(this.hasLogin){
          getNotReadNumber().then(res=>{
            console.log('msgNum',res.data.data.msgNum)
...
/*           if(this.$route.meta.noConsult){
        ...
        Doctor(isDoctor){
          // this.isDoctor = true
          if(isDoctor){
            this.activeMenulist = this.doctorMenulist
            console.log(this.isDoctor)
          }else{
            this.activeMenulist = this.wxAllmenuList
            console.log(this.isDoctor)
          }
        },
        wxToPage(index,item){
          ...
          this.$nextTick(()=>{
            setTimeout(()=>{
              if(index===1){
                let msg=0;
                this.$bridge.callHandler("toLogin", msg, (res) => {
                });
                this.$router_({path:item.userPath,query:{roletype:'0'}})  //医生 1  用户 0
              }else if(index===2){
                let msg=1;
                this.$bridge.callHandler("toLogin", msg, (res) => {
                });
                this.$router_({path:item.userPath,query:{roletype:'1'}})  //医生 1  用户 0
              }else{
