<template>
  <div class="addressEdit_container">
    ...
    <div class="save_button" @click="validateForm">修改地址</div>
  </div>
</template>

<script>
import {updAddress,getAddressDetail} from '@/api/apiz'
export default {
  components: {},
  name: "addressEdit",
  data() {
  ...
  mounted() {
    ...
    this.$nextTick(()=>{
      this.getAddressDetailFun(this.id)
    })
  },
  methods: {
    confirmEvent(val){
    ...
    updAddressFun(){
      ...
      updAddress(Object.assign({id:this.id,isDefault:Number(this.isDefault)},this.formData)).then((res)=>{
    ...
    getAddressDetailFun(id){
      getAddressDetail(id).then(res=>{
        let result = res.data.data
        this.formData=result
        this.isDefault=Boolean(result.isDefault)
        this.area=result.provinceNm+'/'+result.cityNm+'/'+result.districtNm
      })
    }
  }
};
