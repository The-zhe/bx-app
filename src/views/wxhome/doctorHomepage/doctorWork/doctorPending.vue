...
<template>
   <div class="doctorPending">
     ...
     <div class="container" v-for="item in datalist">
       <div class="title">
         <div style="margin-left: 0.25rem">{{item.hjjkServiceOrderVo.serviceName}}</div>
         <div class="t_state" v-show="true">
           <div v-show="item.notic==='新咨询'" class="t_state_new">{{item.notic}}</div>
           <div v-show="item.notic!=='新咨询'" class="t_state_out">{{item.notic}}</div>
         </div>
    ...
    export default {
        ...
        methods:{
          ...
          getinfo(){  //公益区  statCd传  0  待处理传1
            getdoctorWork('4844079179961344',1).then((res)=>{
              this.datalist=res.data.list;
              console.log('workppppp',res);
            })
          }
        },
      mounted() {
