<template>
  <div class="fillsass">
    ...
    <div class="fillill_tip">
      <div class="fillill_tip_one">新冠肺炎可防可治，越早诊断，治疗效果越好，为避免贻误诊治，传染家人，请务必准确填写以下信息！</div>
    </div>
    <div class="fillradius">
    <div class="sassjourney">
      <div class="sassjourney_top">1.近一月内是否：
      ...
      <div class="select"><img :src="is1==='true'?select1:select2" @click="totouch(1,'fkxxOptionsA','tripDesc')">
        <div class="select_name1" :style="is1==='true'?'font-weight:bold':''">Ⅰ.去过湖北、温州等城市</div>
      </div>
      <div class="seleto" v-for="(item,index) in yqdesclist">
        曾居留城市
        <div class="selectocityinp">
          <div class="selecto_city" @click="selectaddcity" style="color:#777777">{{item.area}}</div>
          <div class="yqdelete" @click="toyqdelete(index)"></div>
        </div>
        <van-popup
          v-model="liveaddshow"
          position="bottom"
        ...
        <div class="select_time">
          <div class="select_time_o" @click="showaddstart">{{item.stime}}</div>
          <van-popup
            v-model="isaddstart"
            position="bottom"
          >
            <van-datetime-picker
              ...
              :min-date="minDate"
              :max-date="maxDate"
              @confirm="confirmaddstart"
              @cancel="canceldate"
            />
          </van-popup>
          <div class="s_line"></div>
          <div class="select_time_o" @click="showaddend">{{item.etime}}</div>
          <van-popup
            v-model="isaddend"
            position="bottom"
          >
            <van-datetime-picker
              ...
              :min-date="minDate"
              :max-date="maxDate"
              @confirm="confirmaddend"
              @cancel="canceldate"
            />
          </van-popup>
        </div>
        <div class="select_desc"><textarea :placeholder="ptip" v-model="item.desc"></textarea></div>
      </div>
      ...
      <div class="seleto" v-show="is1==='true'" v-for="(item,index) in yqlist">
        曾居留城市
        <div class="selectocityinp">
        <div class="selecto_city" @click="selectcity" :style="livecity==='null-null-null'?'color:#BCBCBC':'color:#777777'">{{livecity==='null-null-null'?'请输入城市':livecity}}</div>
          <div class="yqdelete" @click="toyqdeletelocal" v-show="yqdesclist.length>0"></div>
      ...
      <div class="select"><img :src="is2==='true'?select1:select2"
        ...
        <div class="select_name2" :style="is2==='true'?'font-weight:bold':''">Ⅱ.接触过来自湖北、温州...的亲友或其他人</div>
      </div>
      ...
      <div class="select"><img :src="is3==='true'?select1:select2"
        ...
        <div class="select_name3" :style="is3==='true'?'font-weight:bold':''">Ⅲ.接触过有以下症状的病人：<br>&emsp;&thinsp;发热37.5℃ 以上且（或）伴有咳嗽、乏<br>&emsp;&thinsp;力、四肢酸痛、头痛、腹泻等</div>
      </div>
      ...
      <div class="select"><img :src="is4==='true'?select1:select2"
        ...
        <div class="select_name4" :style="is4==='true'?'font-weight:bold':''">Ⅳ.是否有前往过人员密集处或封闭环境<br>&ensp;&thinsp;（如电影院、棋牌室、网吧等）</div>
      </div>
      ...
      <div class="cqjz">
        <div class="cqjz_title">*2.长期居住地</div>
        <div class="cqjzinput" @click="selectcqaddress" :style="cqaddress==='null-null-null'?'color:#BCBCBC':'color:#777777'">{{cqaddress==='null-null-null'?'请输入居所':cqaddress }}</div>
        <van-popup
    ...
    <div class="fillbtn" v-show="hideshow">
      ...
      <!--<div class="filltj" @click="totest" v-show="islogin"></div>-->
  ...
  export default {
    ...
    data() {
      return {
        ...
        yqlistsubmit:[],///提交到后台
        cqaddressshow:false,
        cqaddress:localStorage.getItem('proviceNm')+'-'+localStorage.getItem('cityNm')+'-'+localStorage.getItem('districtNm'),
        liveshow: false,
        select1: require('../../../assets/imgs/xfill1_sex1.png'),
        select2: require('../../../assets/imgs/xfill1_sex2.png'),
        num: 4,
        is1: localStorage.getItem('fkxxOptionsA'),
        is2: localStorage.getItem('fkxxOptionsB'),
        is3: localStorage.getItem('fkxxOptionsC'),
        is4: localStorage.getItem('fkxxOptionsD'),
        livecity: localStorage.getItem('tripProviceNm') + '-' + localStorage.getItem('tripCityNm')+'-'+localStorage.getItem('tripAreaNm'),
        stime: localStorage.getItem('tripStartTime'),
        etime: localStorage.getItem('tripEndTime'),
        isstart: false,
        isaddstart:false,
        isend: false,
        isaddend:false,
        ...
        tripDesc: localStorage.getItem('tripDesc'),  // 行程描述
    ...
    methods: {
      totest(){
        console.log('yqlist',this.yqlist);
        console.log('yqdesclist',this.yqdesclist)
      },
      tofocus(){
        this.ptip='';
      },
      ...
      toyqdeletelocal(){
        this.yqlist.splice(0,1);
      },
      yqaddclick(index){
        if(this.livecity==='null-null-null'){
          Toast('请选择居留城市');
        }
        else if(this.stime===''||this.stime===null){
          Toast('请选择居留开始时间');
        }else if(this.etime===''||this.etime===null){
          Toast('请选择居留结束时间');
        }
        else if(this.tripDesc===''||this.tripDesc===null){
          Toast('请填写行程描述')
        }else {
          ...
          localStorage.setItem('tripProviceNm', null);
          localStorage.setItem('tripCityNm', null);
          localStorage.setItem('tripAreaNm',null);
          localStorage.setItem('tripStartTime', '');
          localStorage.setItem('tripEndTime', '');
          localStorage.removeItem('tripDesc');
          this.livecity=null+'-'+null+'-'+null;
          this.stime='';
          this.etime='';
          this.tripDesc='';
      ...
      confirmlive(a) {
        ...
        this.liveshow = false;
        this.livecity = a[0].name + '-' + a[1].name+'-'+a[2].name;
        localStorage.setItem('tripProviceNm', a[0].name);
        localStorage.setItem('tripCityNm', a[1].name);
        localStorage.setItem('tripAreaNm', a[2].name);
        console.log(a);
      },
      confirmstart(value) {
        this.isstart = false;
        this.stime = moment(value).format('YYYY-MM-DD');
        localStorage.setItem('tripStartTime', this.stime);
      },
      confirmend(value) {
        let start=new Date(this.stime);
        let end=new Date(moment(value).format('YYYY-MM-DD'));
        console.log('start',start);
        console.log('end',end);
        if(start>end){
          Toast('居留结束时间不能早于居留开始时间');
        }else {
          this.isend = false;
          this.etime = moment(value).format('YYYY-MM-DD');
          localStorage.setItem('tripEndTime', this.etime);
        }
      },
      confirmaddstart(){
        ...
        if (this.is1 === 'true' && !this.livecity) {
          ...
          if(this.yqlist.length>0){
            this.yqlistsubmit.push({
              ...
              tripAreaNm:localStorage.getItem('tripAreaNm'),
              tripStartTime:localStorage.getItem('tripStartTime'),
              tripEndTime:localStorage.getItem('tripEndTime'),
              tripDesc:this.tripDesc,
            });
          }
          localStorage.setItem('yqlistsubmit',JSON.stringify(this.yqlistsubmit));
          localStorage.setItem('tripDesc', this.tripDesc);
          localStorage.setItem('fkxxOptionsBDesc', this.fkxxOptionsBDesc);
          localStorage.setItem('fkxxOptionsCDesc', this.fkxxOptionsCDesc);
          localStorage.setItem('fkxxOptionsDDesc', this.fkxxOptionsDDesc);
          this.$router_({path: '/fillphone'});
        }
      },
      tosubmit(){
        if (this.is1 === 'true' && !this.livecity||this.livecity==='null-null-null') {
          Toast('请选择居留城市');
        } else if (this.is1 === 'true' && !this.stime) {
          Toast('请选择居留开始时间');
        } else if (this.is1 === 'true' && !this.etime) {
      ...
      updsubmit(){
      /*  if(localStorage.getItem('yqlistsubmit')==='[]'){
          let aa=[{}];
          localStorage.setItem('yqlistsubmit',JSON.stringify(aa));
        }*/
        let hjjkYqConsultRo = {
          nickname:localStorage.getItem('nickname'),
          id:Number(localStorage.getItem('consultId')),
          hjjkYqTripVos:JSON.parse(localStorage.getItem('yqlistsubmit')),
          jqDistrictNm:localStorage.getItem('jqDistrictNm'),
          districtNm:localStorage.getItem('districtNm'),
          birthday: localStorage.getItem('birthday'),
      ...
      emptytable() {
        ...
        localStorage.removeItem('tripAreaNm');
  ...
  .fillsass {
    ...
    .fillprogress{
      ...
      .fill_b{
        ...
        width: 5.92rem;
        height: 0.82rem;
        background: url("../../../assets/imgs/x_fill_s1.png") no-repeat 0 0.22rem;
    ...
    .fillill_tip{
      ...
      .fillill_tip_one{
        ...
        background: url("../../../assets/imgs/x_fill_s2.png") no-repeat 0 0.35rem;
    ...
    .sassjourney {
      ...
      .sassjourney_top {
        ...
        .top_sass {
          ...
          width: 2.2rem;
          height: 0.6rem;
          background: url("../../../assets/imgs/x_fill_s3.png") no-repeat;
          background-size: 1.95rem 0.6rem;
        }
      }
      ...
      .seleto {
        ...
        .selecto_city{
          ...
          border-radius: 0.08rem;
          border: 0.02rem solid #D6D6D8;
          background: url('../../../assets/imgs/x_fill_b2.png') no-repeat 3.45rem;
          background-size: 0.21rem 0.16rem;
        ...
        .yqdelete{
          ...
          width: 0.44rem;
          height: 0.44rem;
          background: url("../../../assets/imgs/x_fillsassdelete.png");
          background-size: 0.44rem 0.44rem;
        }
        ...
        .select_time {
          ...
          .select_time_o {
            ...
            border-radius: 0.08rem;
            border: 0.02rem solid #D6D6D8;
            background: url("../../../assets/imgs/xfill1_date.png") no-repeat 1.95rem;
